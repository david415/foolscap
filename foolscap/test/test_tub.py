...
class SetLocation(unittest.TestCase):
    ...
    def test_set_location(self):
        ...
        t.setLocation("tcp:host=127.0.0.1:port=12345")
        # setLocation may only be called once
        self.failUnlessRaises(PBError, t.setLocation, "tcp:host=127.0.0.1:port=12345")
    ...
    def test_set_location_automatically(self):
        ...
        def _check(furl):
            ...
            if sr.encrypted:
                self.failUnless( "tcp:host=127.0.0.1:port=%s" % (portnum,) in sr.locationHints)
            else:
